...
@AndroidEntryPoint
class PendingOrderFragment : BaseFragment<BaseViewModal , FragmentOrderBinding>() , OrderListAdapter.OnItemSelected{
    ...
    fun getOrderList(pageno :Int) {
        ...
        mViewModel.order(session.user.result.get(0).userid!!.toInt(), pageno,0).observe(requireActivity(), Observer {
            it?.let { resource ->
